...
TELEGRAM_TOKEN = "8765917467:AAEzbEphXQAEJ0hdFFwW1p_m3aI4MUE7v7k"
TELEGRAM_CHAT_ID = "-1003965435761"  # Canal: 2M Trading signals
TELEGRAM_ADMIN_ID = "1028710661"  # Admin pour notifications privées
...
# Indices OTC (disponibles 24/7 sur Deriv)
OTC_INDICES = {
    'OTC_DJI':   {'name': 'US30 (Dow Jones)', 'symbol': 'OTC_DJI',   'type': 'otc_index', 'tp_ticks': 150, 'sl_ticks': 100, 'timeframe': '5-15 min'},
    'OTC_FCHI':  {'name': 'CAC40',            'symbol': 'OTC_FCHI',  'type': 'otc_index', 'tp_ticks': 80,  'sl_ticks': 50,  'timeframe': '5-15 min'},
    'OTC_GDAXI': {'name': 'DAX',              'symbol': 'OTC_GDAXI', 'type': 'otc_index', 'tp_ticks': 100, 'sl_ticks': 60,  'timeframe': '5-15 min'},
    'frxXAUUSD': {'name': 'Gold (XAU/USD)',   'symbol': 'frxXAUUSD', 'type': 'commodity', 'tp_ticks': 5.0, 'sl_ticks': 3.0, 'timeframe': '5-15 min'},
}

# Tous les instruments
ALL_INSTRUMENTS = {**VOLATILITY_INDICES, **FOREX_PAIRS, **CRASH_BOOM_INDICES, **OTC_INDICES}
...
def send_telegram_message(message):
    """Envoie un message sur Telegram (canal + admin privé)"""
    try:
        url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage"
        sent = 0
        
        # Envoyer sur le canal public
        try:
            payload_channel = {
            ...
            response = requests.post(url, json=payload_channel, timeout=10)
            if response.json().get('ok'):
                sent += 1
            else:
                print(f"  Canal Telegram erreur: {response.json().get('description', 'unknown')}")
        except Exception as e:
            print(f"  Canal Telegram: {e}")
        
        # Envoyer aussi à l'admin en privé
        try:
            payload_admin = {
            ...
            response = requests.post(url, json=payload_admin, timeout=10)
            if response.json().get('ok'):
                sent += 1
            else:
                print(f"  Admin Telegram erreur: {response.json().get('description', 'unknown')}")
        except Exception as e:
            print(f"  Admin Telegram: {e}")
...
def send_telegram_file(file_path, caption=""):
    """Envoie un fichier sur Telegram (canal + admin privé)"""
    try:
